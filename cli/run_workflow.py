#!/usr/bin/env python3
"""
Run Steps 1-4 locally (no Docker) for one or all images.

Usage examples:
    # Full pipeline — fetch 1 image + segment + PPC + upload
    python run_workflow.py --config dsa_workflow_config.json --root-folder-id 67ddb0782fc8ce397c5ef7fb --steps 1,2,3,4

    # Fetch folder + run steps 2,3 on ALL images (no upload)
    python run_workflow.py --config dsa_workflow_config.json --root-folder-id 67ddb0782fc8ce397c5ef7fb --steps 1,2,3

    # Reuse cached list (skip step 1), run steps 2,3 on all
    python run_workflow.py --config dsa_workflow_config.json --steps 2,3 --all-items

    # Upload only (step 4), uses cached item list
    python run_workflow.py --config dsa_workflow_config.json --steps 4 --all-items

    # Force reprocess everything
    python run_workflow.py --config dsa_workflow_config.json --root-folder-id 67ddb0782fc8ce397c5ef7fb --steps 1,2,3,4 --force
"""
...
def _subprocess_env():
    """Return an environment dict that adds user site-packages to PYTHONPATH
    so subprocesses (TissueSegmentation, PPC) can find torch/transformers."""
    env = os.environ.copy()
...
def _run_step1_fetch_dsa(config, root_folder_id=None):
    ...
    for i, item in enumerate(filtered):  # Show all items
        try:
            item_path = _build_item_path(gc, item)
            item['fullPath'] = item_path
        except Exception as e:
...
def _run_step2_verify_local(config, dsa_items):
    ...
    print(f"✓ Step 2 complete: {matched_count}/{len(enriched)} items have local matches")
...
# Main
# ---------------------------
def main(config_path: str, item_id: str = None, all_items: bool = False, force: bool = False, run_steps: str = "all", root_folder_id: str = None):
    """
    Execute workflow (Steps 1-4):
    Step 1: Fetch items from DSA folder + verify local files  (requires --root-folder-id)
    ...
    # Validate inputs for steps 2-4
    if steps_to_run & {"2", "3", "4"}:
        # If no item_id specified, automatically process all items
        if not item_id:
            all_items = True
            print("No item-id specified, will process all items from step-1 output.")
    ...
    if all_items:
        ...
        print(f"Will process all {len(to_process)} items from the step-1 list.")
    else:
        # Filter to the single requested item
        to_process = [r for r in all_images if _match_item(r, item_id)]
        if not to_process:
            # Item not in cached list: try fetching this single item from DSA
            print(f"Item {item_id} not in cached list. Fetching from DSA...")
            to_process = _fetch_and_enrich_one_item(config, item_id)
            if not to_process:
                available = [r.get("_id", r.get("itemId", "N/A")) for r in all_images[:10]]
                raise SystemExit(
                    f"No record for item_id {item_id} in step-1 output. "
                    f"Available item IDs: {available}"
                )

    # Load parameters for Steps 2-4
    s2 = config["step2"]
    s3 = config["step3"]
    s4 = config.get("step4", {})
    ...
    # Accumulate results for all processed images
    results = []

    for rec in to_process:
        if not rec.get("has_local_match"):
            print(f"[SKIP] No local match for item {rec.get('_id', '?')}")
            continue
        ...
        # Skip if BOTH outputs already exist (job already completed) and not forced
        ...
        if "4" in steps_to_run:
            if ppc_out_path.exists() and (out_dir / ppc_anot).exists():
                ...
                ok = _run_step4_upload(
                    config_path,
                    config,
                    rec['_id'],          # use the current item's ID
                    ppc_out_path,
                    out_dir / ppc_anot,
                )
    ...
    # Save final summary for all processed images (separate from the step-1 items list)
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(
        description="Run Steps 1-4 locally (no Docker) for one or all images."
    )
    ...
    ap.add_argument("--item-id", default=None, help="Single DSA item _id to process (Steps 2-4)")
    ap.add_argument("--all-items", action="store_true", help="Process all items from cached step-1 list")
    ap.add_argument("--force", action="store_true", help="Force reprocessing, ignore existing outputs")
    ap.add_argument("--steps", default="all", help="Steps to run: '1', '2', '3', '4', '1,2', '2,3', '3,4', or 'all'")
    args = ap.parse_args()
    main(args.config, item_id=args.item_id, all_items=args.all_items, force=args.force, run_steps=args.steps, root_folder_id=args.root_folder_id)